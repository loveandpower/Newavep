	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			if data.current.value == 'citizen_wear' then
				...
				TriggerServerEvent("player:serviceOff", "pecheur")
...
function OpenPecheurActionsMenu()
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		local elements = {
			{label = 'Renault Master',  value = 'boxville'},
		}
		...
		if Config.EnablePlayerManagement and PlayerData.job ~= nil and (PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'chef' or PlayerData.job.grade_name == 'experimente') then
              table.insert(elements, {label = 'Véhicule Patron', value = 'slamvan3'})
        end
...
function OpenVehicleSpawner2Menu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint2.Pos, 90.0, function(vehicle)
						ESX.Game.SetVehicleProperties(vehicle, vehicleProps)
						local playerPed = GetPlayerPed(-1)
						TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
					end)
				...
				function(data, menu)
					...
					CurrentActionMsg  = _U('spawn_veh2')
					CurrentActionData = {}
		...
		local elements = {
			{label = 'Bateau de Pêche',  value = 'tug'},
			{label = 'Bateau ',  value = 'Dinghy'},
		}
		...
		ESX.UI.Menu.Open(
			...
			function(data, menu)
				...
				ESX.Game.SpawnVehicle(model, Config.Zones.VehicleSpawnPoint2.Pos, 56.326, function(vehicle)
					local playerPed = GetPlayerPed(-1)
					TaskWarpPedIntoVehicle(playerPed,  vehicle,  -1)
				end)
			end,
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_pecheur', 'Pécheur', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_pecheurjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count',
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_pecheurjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_pecheurjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_pecheurjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_pecheurjob:hasEnteredMarker', function(zone)
	if zone == 'SaumonFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'saumon_harvest'
		CurrentActionMsg  = _U('press_collect1')
		CurrentActionData = {zone= zone}
	end

	if zone == 'ThonFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'thon_harvest'
		CurrentActionMsg  = _U('press_collect2')
		CurrentActionData = {zone= zone}
	end

	if zone == 'CrevetteFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'crevette_harvest'
		CurrentActionMsg  = _U('press_collect3')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementSaumon' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'saumon_traitement'
		CurrentActionMsg  = _U('press_traitement1')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementThon' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'thon_traitement'
		CurrentActionMsg  = _U('press_traitement2')
		CurrentActionData = {zone = zone}
	end
	if zone == 'TraitementCrevette' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur'  then
		CurrentAction     = 'crevette_traitement'
		CurrentActionMsg  = _U('press_traitement3')
		CurrentActionData = {zone = zone}
	end
	...
	if zone == 'PecheurActions' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		CurrentAction     = 'pecheur_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
	...
	if zone == 'VehicleSpawner2' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		CurrentAction     = 'vehicle_spawner_menu2'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
	...
	if zone == 'VehicleDeleter2' and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
...
AddEventHandler('esx_pecheurjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'SaumonFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopHarvest1')
	end
	if (zone == 'ThonFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopHarvest2')
	end
	if (zone == 'CrevetteFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopHarvest3')
	end
	if (zone == 'TraitementSaumon') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopTransform1')
	end
	if (zone == 'TraitementThon') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopTransform2')
	end
	if (zone == 'TraitementCrevette') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
		TriggerServerEvent('esx_pecheurjob:stopTransform3')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' then
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'pecheur' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'saumon_harvest' then
					TriggerServerEvent('esx_pecheurjob:startHarvest1', CurrentActionData.zone)
				end
				if CurrentAction == 'thon_harvest' then
					TriggerServerEvent('esx_pecheurjob:startHarvest2', CurrentActionData.zone)
				end
				if CurrentAction == 'crevette_harvest' then
					TriggerServerEvent('esx_pecheurjob:startHarvest3', CurrentActionData.zone)
				end
				if CurrentAction == 'saumon_traitement' then
					TriggerServerEvent('esx_pecheurjob:startTransform1', CurrentActionData.zone)
				end
				if CurrentAction == 'thon_traitement' then
					TriggerServerEvent('esx_pecheurjob:startTransform2', CurrentActionData.zone)
				end
				if CurrentAction == 'crevette_traitement' then
					TriggerServerEvent('esx_pecheurjob:startTransform3', CurrentActionData.zone)
				end
