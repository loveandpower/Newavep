	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('vine_clothes_vine'), value = 'boulan_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'boulan_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'boulan_actions_menu'
...
function OpenBoulanActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'boulan_actions',
		{
			title    = 'boulan',
			align    = 'top-left',
			elements = elements
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'boulan', function(data, menu)
					menu.close()
				end)
			end
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'boulan', vehicleProps)
...
function OpenMobileboulanActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_boulan_actions',
		{
			title    = 'boulan',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_boulanger', 'Boulanger', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_boulanger:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_boulanger:getStockItem', itemName, count)
							-----------------
							Citizen.Wait(500)
							OpenGetStocksMenu()
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_boulanger:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_boulanger:putStockItems', itemName, count)
							-----------------
							Citizen.Wait(500)
							OpenPutStocksMenu()
						end
...
AddEventHandler('esx_boulanger:hasEnteredMarker', function(zone)
	if zone == 'BleFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'boulan'  then
		CurrentAction     = 'ble_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementFarine' and PlayerData.job ~= nil and PlayerData.job.name == 'boulan'  then
		CurrentAction     = 'ble_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	if zone == 'TraitementLevain' and PlayerData.job ~= nil and PlayerData.job.name == 'boulan'  then
		CurrentAction     = 'farine_traitement'
		CurrentActionMsg  = _U('press_traitementL')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'SellFarm1' and PlayerData.job ~= nil and PlayerData.job.name == 'boulan'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'BoulangerActions' and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then
		CurrentAction     = 'boulan_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then
...
AddEventHandler('esx_boulanger:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'BleFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then
		TriggerServerEvent('esx_boulanger:stopHarvest')
	end  
	if (zone == 'TraitementFarine' or zone == 'TraitementLevain') and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then
		TriggerServerEvent('esx_boulanger:stopTransform')
	end
	if (zone == 'SellFarm1') and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then
		TriggerServerEvent('esx_boulanger:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.BoulangerActions.Pos.x, Config.Zones.BoulangerActions.Pos.y, Config.Zones.BoulangerActions.Pos.z)
		...
		AddTextComponentString("Boulanger")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then

		for k,v in pairs(Config.Zones)do
			if v.Type == 1 then
				local blip2 = AddBlipForCoord(v.Pos.x, v.Pos.y, v.Pos.z)

				SetBlipSprite (blip2, 171)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'boulan' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_boulanger:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_boulanger:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'ble_harvest' then
					TriggerServerEvent('esx_boulanger:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'ble_traitement' then
					TriggerServerEvent('esx_boulanger:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farine_traitement' then
					TriggerServerEvent('esx_boulanger:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_boulanger:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'boulan_actions_menu' then
					OpenBoulanActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'boulan', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'boulan' and (GetGameTimer() - GUI.Time) > 150 then
