...
AddEventHandler('esx_scoreboard:updateConnectedPlayers', function(connectedPlayers)
	UpdatePlayerTable(connectedPlayers)
	---function change nb ped
	GeneratePedFuncPlayer()
end)
...
function GeneratePedFuncPlayer()
	...
	if onlinePlayers < 5 then
		...
		--Citizen.CreateThread(function()
		--	while true do
				SetVehicleDensityMultiplierThisFrame(traficDensity)
				SetPedDensityMultiplierThisFrame(pedDensity)
				SetRandomVehicleDensityMultiplierThisFrame(traficDensity)
				SetParkedVehicleDensityMultiplierThisFrame(traficDensity)
