	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			elements = {
				...
				{label = _U('vine_clothes_vine'), value = 'mineur_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'mineur_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'mineur_actions_menu'
...
function OpenmineurActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mineur_actions',
		{
			title    = 'Mineur',
			align    = 'top-left',
			elements = elements
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'mineur', vehicleProps)
...
function OpenMobilemineurActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_mineur_actions',
		{
			title    = 'mineur',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_mineur', 'mineur', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_mineurjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_mineurjob:getStockItem', itemName, count)
							-----------------
							Citizen.Wait(500)
							OpenGetStocksMenu()
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_mineurjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_mineurjob:putStockItems', itemName, count)
							----------------
							Citizen.Wait(500)
							OpenPutStocksMenu()
						end
...
AddEventHandler('esx_mineurjob:hasEnteredMarker', function(zone)
	if zone == 'Minefarm' and PlayerData.job ~= nil and PlayerData.job.name == 'mineur'  then
		CurrentAction     = 'rock_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementMine' and PlayerData.job ~= nil and PlayerData.job.name == 'mineur'  then
		CurrentAction     = 'rock_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end		
		
	--if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'mineur'  then
	--	CurrentAction     = 'rock_traitement'
	--	CurrentActionMsg  = _U('press_traitement')
	--	CurrentActionData = {zone = zone}
	--end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'mineur'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'mineurActions' and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
		CurrentAction     = 'mineur_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
...
AddEventHandler('esx_mineurjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'Minefarm') and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
		TriggerServerEvent('esx_mineurjob:stopHarvest')
	end  
	--if (zone == 'TraitementMine' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
	if (zone == 'TraitementMine') and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
		TriggerServerEvent('esx_mineurjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
		TriggerServerEvent('esx_mineurjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Entreprise Mine")
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_mineurjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_mineurjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'rock_harvest' then
					TriggerServerEvent('esx_mineurjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'rock_traitement' then
					TriggerServerEvent('esx_mineurjob:startTransform', CurrentActionData.zone)
				end
				--if CurrentAction == 'tabacsec_traitement' then
				--	TriggerServerEvent('esx_mineurjob:startTransform', CurrentActionData.zone)
				--end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_mineurjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'mineur_actions_menu' then
					OpenmineurActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'mineur', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobilemineurActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)