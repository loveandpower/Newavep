...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			...
			CurrentAction     = 'pompe_actions_menu'
...
function OpenmineurActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'pompe_actions',
		{
			title    = 'Pompsite',
			align    = 'top-left',
			elements = elements
...
function OpenMobilemineurActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_pompe_actions',
		{
			title    = 'mineur',
			align    = 'top-left',
			elements = {
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_pompiste:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_pompiste:getStockItem', itemName, count)
							-----------------
							Citizen.Wait(500)
							OpenGetStocksMenu()
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_pompiste:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_pompiste:putStockItems', itemName, count)
							----------------
							Citizen.Wait(500)
							OpenPutStocksMenu()
						end
...
AddEventHandler('esx_pompiste:hasEnteredMarker', function(zone)
	...
	if zone == 'PetrolTransform' and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste'  then
	...
	if zone == 'PetrolActions' and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' then
		CurrentAction     = 'pompe_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_pompiste:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'PetrolFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' then
		TriggerServerEvent('esx_pompiste:stopHarvest')
	end  
	--if (zone == 'TraitementMine' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'mineur' then
	if (zone == 'PetrolTransform') and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' then
		TriggerServerEvent('esx_pompiste:stopTransform')
	end
	--if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' then
	--	TriggerServerEvent('esx_pompiste:stopSell')
	--end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		...
		local blip = AddBlipForCoord(1374.4848632813, -2073.1962890625, 51.998519897461)
		...
		AddTextComponentString("Entreprise Pompe")
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'pompiste' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'rock_harvest' then
					TriggerServerEvent('esx_pompiste:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'rock_traitement' then
					TriggerServerEvent('esx_pompiste:startTransform', CurrentActionData.zone)
				end
				...
				--	TriggerServerEvent('esx_pompiste:startSell', CurrentActionData.zone)
				--end
				
				if CurrentAction == 'pompe_actions_menu' then
