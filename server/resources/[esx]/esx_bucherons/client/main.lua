...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('buche_clothes_civil'), value = 'citizen_wear'},
				{label = _U('buche_clothes_buche'), value = 'buche_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'buche_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
...
function OpenbucheActionMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'buche_actions',
		{
			title    = 'Bucherons',
			align    = 'top-left',
			elements = elements
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'buche', vehicleProps)
...
function OpenMobilebucheActionMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_buche_actions',
		{
			title    = 'Bucherons',
			align    = 'top-left',
			elements = {
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_bucherons:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_bucherons:putStockItems', itemName, count)
							----------------
							Citizen.Wait(500)
							OpenPutStocksMenu()
						end
...
AddEventHandler('esx_bucherons:hasEnteredMarker', function(zone)
	if zone == 'woodFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'buche'  then
		CurrentAction     = 'feuilletab_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementBois' and PlayerData.job ~= nil and PlayerData.job.name == 'buche'  then
		CurrentAction     = 'buche_traitement'
		CurrentActionMsg  = _U('press_collect2')
		CurrentActionData = {zone= zone}
	end		
		
--	if zone == 'TraitementJus' and PlayerData.job ~= nil and PlayerData.job.name == 'buche'  then
--		CurrentAction     = 'jus_traitement'
--		CurrentActionMsg  = _U('press_traitement')
--		CurrentActionData = {zone = zone}
--	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'buche'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'bucheAction' and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
		CurrentAction     = 'buche_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
...
AddEventHandler('esx_bucherons:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'woodFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
		TriggerServerEvent('esx_bucherons:stopHarvest')
	end  
--	if (zone == 'TraitementBois' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
	if (zone == 'TraitementBois') and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
		TriggerServerEvent('esx_bucherons:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
		TriggerServerEvent('esx_bucherons:stopSell')
	end
	CurrentAction = nil
end)
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'buche' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_bucherons:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_bucherons:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'buche' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'feuilletab_harvest' then
					TriggerServerEvent('esx_bucherons:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'jus_traitement' then
					TriggerServerEvent('esx_bucherons:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'buche_traitement' then
					TriggerServerEvent('esx_bucherons:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_bucherons:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'buche_actions_menu' then
					OpenbucheActionMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'buche', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'buche' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobilebucheActionMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)