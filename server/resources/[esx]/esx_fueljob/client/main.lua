...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('wine_clothes_civil'), value = 'citizen_wear'},
				{label = _U('wine_clothes_wine'), value = 'fueler_wear'},
			},
		},
		function(data, menu)
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_fueler', 'Raffineur', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_fueljob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_fueljob:getStockItem', itemName, count)
							-----------------
							Citizen.Wait(500)
							OpenGetStocksMenu()
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_fueljob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_fueljob:putStockItems', itemName, count)
							----------------
							Citizen.Wait(500)
							OpenPutStocksMenu()
						end
...
AddEventHandler('esx_fueljob:hasEnteredMarker', function(zone)
	if zone == 'PetroleFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'fueler'  then
		CurrentAction     = 'petrol_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end	

	if zone == 'TraitementRaffine' and PlayerData.job ~= nil and PlayerData.job.name == 'fueler'  then
		CurrentAction     = 'petrole_traitement'
		CurrentActionMsg  = _U('press_traitement_raffine')
		CurrentActionData = {zone = zone}
	end
		
	if zone == 'TraitementPetrole' and PlayerData.job ~= nil and PlayerData.job.name == 'fueler'  then
		CurrentAction     = 'petrole_traitement'
	...
	if zone == 'RaffineurActions' and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		CurrentAction     = 'fueler_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
...
AddEventHandler('esx_fueljob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'PetroleFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		TriggerServerEvent('esx_fueljob:stopHarvest')
	end  
	if (zone == 'TraitementPetrole') and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		TriggerServerEvent('esx_fueljob:stopTransform')
	end
	if (zone == 'TraitementRaffine') and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		TriggerServerEvent('esx_fueljob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
		TriggerServerEvent('esx_fueljob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		...
		AddTextComponentString("Raffineur")
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'fueler' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_fueljob:hasEnteredMarker', currentZone)
			end
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'petrol_harvest' then
					TriggerServerEvent('esx_fueljob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'petrole_traitement' then
					TriggerServerEvent('esx_fueljob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_fueljob:startSell', CurrentActionData.zone)
				end
		...
		if IsControlPressed(0,  Keys['F6']) and PlayerData.job ~= nil and PlayerData.job.name == 'fueler' and (GetGameTimer() - GUI.Time) > 150 then
...
RegisterNetEvent('NB:openMenuFueler')
AddEventHandler('NB:openMenuFueler', function()
	OpenMobileFuelerActionsMenu()
end)