...
-- enter/exit zone
AddEventHandler('esx_garbadge:hasEnteredMarker', function(zone)
  printDebug('hasEnteredMarker: ' .. zone.name)
  if zone.name == 'cloakRoom' then
    currentAction = 'Littleprikmenu'
  ...
  elseif zone.name == 'northBank' then
    ...
    currentActionData = {}
  elseif zone.name == 'decheterie' then
...
AddEventHandler('esx_garbadge:hasExitedMarker', function(zone)
  printDebug('hasExitedMarker: ' .. zone.name)
  if zone.name == 'market' then
    TriggerServerEvent('esx_garbadge:stopHarvestRun') 
  elseif zone.name == 'bank' then 
    TriggerServerEvent('esx_garbadge:stopSellRun') 
  elseif zone.name == 'northBank' then
    TriggerServerEvent('esx_garbadge:stopWeeklyCollect') 
  elseif zone.name == 'unionDepository' then
    TriggerServerEvent('esx_garbadge:stopWeeklyDestruct') end
    currentAction    = nil
    currentActionMsg = ''
  ...
  while true do
    ...
    if playerData.job.name == Config.jobName then
      ...
      if isInMarker and not alreadyInZone then
        ...
        TriggerEvent('esx_garbadge:hasEnteredMarker', currentZone)
      end
      if not isInMarker and alreadyInZone then
        alreadyInZone = false
        TriggerEvent('esx_garbadge:hasExitedMarker', lastZone)
      end
    end
  end
end)
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if IsControlJustReleased(1, Keys["F6"]) and playerData.job.name == Config.jobName then openMobileLittleprikmenu() end
    if currentAction ~= nil then
      ...
      if IsControlJustReleased(1, Keys["E"]) then
        ...
        if currentAction     == 'Littleprikmenu' then
    
          if not inVehicle then 
            openPricksActionsMenu() 
          else 
        ...
        elseif currentAction == 'harvestRun' then
          ...
          if not inVehicle then
            TriggerServerEvent('esx_garbadge:startHarvestRun')
          else
            ESX.ShowNotification(_U('in_vehicle')) 
          end
        ...
        elseif currentAction == 'sellRun' then
          ...
          if not inVehicle then
            TriggerServerEvent('esx_garbadge:startSellRun')
          else 
            ESX.ShowNotification(_U('in_vehicle')) 
          end
        ...
        elseif currentAction == 'collectWeekly' then
          ...
          if not inVehicle then 
            if playerData.job.grade >= Config.weeklyMinGrade then
              TriggerServerEvent('esx_garbadge:startWeeklyCollect')
            else
              ESX.ShowNotification(_U('bad_grade'))
            end 
          else
           ESX.ShowNotification(_U('in_vehicle'))
          end
        ...
        elseif currentAction == 'destructWeekly' then
          ...
          if not inVehicle then 
            if playerData.job.grade >= Config.weeklyMinGrade then
              TriggerServerEvent('esx_garbadge:startWeeklyDestruct')
            else
              ESX.ShowNotification(_U('bad_grade'))
            end 
          else
...
-- menu brinks
function openPricksActionsMenu()
  printDebug('openPricksActionsMenu')
  local elements = {}  
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      menu.close()
      TriggerEvent('esx_garbadge:hasEnteredMarker', lastZone)
    end
  )
end

function openPriksStorageMenu()
  printDebug('openPriksStorageMenu')
  local elements = {}    
...
function openGetStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_garbadge:getStockItems', function(items)
    local elements = {}
    for i=1, #items, 1 do
        ...
        ESX.UI.Menu.Open(
          ...
          function(data2, menu2)
            ...
            if count == nil then
              ...
              menu2.close()
              menu.close()
              TriggerServerEvent('esx_garbadge:getStockItem', itemName, count)
              openGetStocksMenu()
            end
          end,
          function(data2, menu2)
...
function openPutStocksMenu()
  ...
  ESX.TriggerServerCallback('esx_garbadge:getPlayerInventory', function(inventory)
    local elements = {}
          ...
          ESX.UI.Menu.Open(
            ...
            function(data2, menu2)
              ...
              if count == nil then
                ...
                menu2.close()
                menu.close()
                TriggerServerEvent('esx_garbadge:putStockItems', itemName, count)
                openPutStocksMenu()
              end
            end,
            function(data2, menu2)
...
function openGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_garbadge:getArmoryWeapons', function(weapons)
    local elements = {}
    ...
    function(data, menu)
      if data.current.value ~= 'empty' then
        menu.close()
        ESX.TriggerServerCallback('esx_garbadge:removeArmoryWeapon', function() openGetWeaponMenu() end, data.current.value)
      end
    end, 
    function(data, menu)
      menu.close()
...
function openPutWeaponMenu()
  ...
  function(data, menu)
    if data.current.value ~= 'empty' then
      menu.close()
      ESX.TriggerServerCallback('esx_garbadge:addArmoryWeapon', function() openPutWeaponMenu() end, data.current.value, true)
    end
    ...
    function(data, menu)
      ...
      if amount == nil then
        ...
        if closestPlayer == -1 or closestDistance > 3.0 then
          ...
          TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_Litlle_Pricks', 'Litlle_Pricks', amount)
        end
      end
    end,
  function(data, menu)
...
function openMobileLittleprikmenu()
  printDebug('openMobileLittleprikmenu')
  ESX.UI.Menu.CloseAll()
  ESX.UI.Menu.Open(
...
RegisterNetEvent('esx_garbadge:nextMarket')
AddEventHandler('esx_garbadge:nextMarket', function()
  local tmpList = {}
