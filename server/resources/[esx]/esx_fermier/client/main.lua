...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('ferme_clothes_civil'), value = 'citizen_wear'},
				{label = _U('ferme_clothes_ferme'), value = 'ferme_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'ferme_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
...
function OpentabacActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'ferme_actions',
		{
			title    = 'ferme',
			align    = 'top-left',
			elements = elements
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'ferme', vehicleProps)
...
function OpenMobilefermeActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_ferme_actions',
		{
			title    = 'Ferme',
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_ferme', 'ferme', amount)
								end)
							end
						end
					end,
...
AddEventHandler('esx_fermier:hasEnteredMarker', function(zone)
	if zone == 'blefarm' and PlayerData.job ~= nil and PlayerData.job.name == 'ferme'  then
...
AddEventHandler('esx_fermier:hasExitedMarker', function(zone)
	...
	if (zone == 'TraitementBle' or zone == 'TraitementJus') and PlayerData.job ~= nil and PlayerData.job.name == 'ferme' then
		TriggerServerEvent('esx_fermier:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'ferme' then
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'ferme' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'ble_harvest' then
					TriggerServerEvent('esx_fermier:startHarvest', CurrentActionData.zone)
				end
